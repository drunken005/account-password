...
class Account {
    ...
    async loginWithPassword(options) {
        ...
        if (options.hasOwnProperty('email')) {
            ...
            user = await self.account.findOne({'emails.address': options.email});
        } else {
            check(options, {
                username: {type: 'string'},
                password: {type: 'string'}
            });
            user = await self.account.findOne({username: options.username});
        }
        if (!user) {
            throw new Error(`User with that ${isEmail && 'email address' || 'username'} not found.`);
        }
    ...
    async loginWithToken(options) {
        ...
        if (!user) {
            throw new Error('You\'ve been logged out by the server. Please log in again.');
        }
    ...
    async changePassword(userId, oldPassword, newPassword) {
        check({userId, oldPassword, newPassword},
            {
        ...
        return await self.account.updateOne({_id: userId}, {
    ...
    async resetPassword(token, newPlaintextPassword) {
        const self = this;
        check({token, newPlaintextPassword},
            {
                token: {type: 'string', require: true},
                newPlaintextPassword: {type: 'string', require: true},
            });
    ...
    async setPassword(userId, newPlaintextPassword) {
        ...
        return await self.account.updateOne({_id: user._id}, update);
    }
}
